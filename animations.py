...
def new(animation_name):
    if animation_name == 'IDLE':
        ...
        return Animation('RIGHT_ATTACK', (
            Iterator([(abs(TILE - i) - TILE, 0) for i in range(0, 2 * TILE, 4)], TILE // 2)))
    elif animation_name == 'LEFT_ATTACK':
        return Animation('LEFT_ATTACK',
                         Iterator([(TILE - abs(TILE - i), 0) for i in range(0, 2 * TILE, 4)],
                                  TILE // 2))
    elif animation_name == 'UP_ATTACK':
        return Animation('UP_ATTACK',
                         Iterator([(0, TILE - abs(TILE - i)) for i in range(0, 2 * TILE, 4)],
                                  TILE // 2))
    elif animation_name == 'DOWN_ATTACK':
        return Animation('DOWN_ATTACK',
                         Iterator([(0, abs(TILE - i) - TILE) for i in range(0, 2 * TILE, 4)],
                                  TILE // 2))
